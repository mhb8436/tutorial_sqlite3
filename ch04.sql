-- 회원(customer) 테이블에서 전체 행갯수를 구해보세요
SELECT COUNT(*), 
       COUNT(cust_id), 
       COUNT(city), 
       COUNT(region)
...
-- 회원(customer) 테이블에서 도시별 총 마일리지를 구하기 (조건: 회사이름에 T 자가 들어가고, 도시별 마일리지가 총합이 100 이상)
SELECT city, 
...
SELECT GROUP_CONCAT(DISTINCT region, ', ') AS regions
FROM customer;

-- 회원(customer) 테이블에서 도시별 회사이름을 콤마(,)로 연결하여 출력하기  
SELECT city, 
       GROUP_CONCAT(company_name, ', ') AS customer_companies
FROM customer
GROUP BY city;
...
SELECT STRFTIME('%Y', order_date) AS order_year, 
       (CAST((STRFTIME('%m', order_date) - 1) / 3 + 1 AS INTEGER)) AS quarter, 
       COUNT(*) AS order_count
FROM orders
...
-- 고객 (customer) 테이블에서 마일리지가 100 이상이면 VIP 고객, 아니면 일반 고객으로 나누어 회원수와 평균 마일리지를 구하기 
SELECT CASE 
