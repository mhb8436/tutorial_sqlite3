...
-- 사원 정보 추가 
INSERT INTO employee (emp_no, name, eng_name, position, gender, birth_date, hire_date, address, city, region, home_phone, manager_no, dept_id) 
VALUES
...
-- 제품 정보 추가, 제품 아이디가 같으면 업데이트 아니면 추가 (SQLite에서는 ON CONFLICT가 INSERT에만 적용됨)
INSERT OR REPLACE INTO product (product_id, product_name, unit_price, stock)
...
-- insert select 구문으로 요약 테이블 정보 추가 
...
UPDATE product
SET unit_price = (SELECT AVG(unit_price) FROM product WHERE product_name LIKE '%소스%')
WHERE product_id = 91;
...
-- 주문 상세에 없는 주문 번호는 모두 삭제 (SQLite에서는 DELETE에 JOIN 사용 불가)
DELETE FROM orders
...
-- 주문 정보가 없는 고객을 모두 삭제 (JOIN을 사용할 수 없으므로 서브쿼리 활용)
...
-- 제품별 주문 수량 합계 뷰 생성
CREATE VIEW IF NOT EXISTS view_product_order_quantity_sum AS
...
-- 여성 사원 정보를 담은 뷰 생성
CREATE VIEW IF NOT EXISTS view_female_employees AS
SELECT name AS 이름,
       home_phone AS 전화번호,
       hire_date AS 입사일,
       address AS 주소,
